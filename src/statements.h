...
// SQL SELECT
#define SQL_STATEMENT_SELECT_USER_RECORDS_WITH_DIFFICULTY(difficulty) sqlite3_mprintf("SELECT %s, %s, %s FROM %s INNER JOIN %s ON %s.%s = %s.%s WHERE %s = %i ORDER BY %s ASC;", TBL_USERS_COLUMN_Nickname, TBL_SCORE_COLUMN_Time, TBL_SCORE_COLUMN_Difficulty, TABLENAME_SCORES, TABLENAME_USERS, TABLENAME_USERS, TBL_USERS_COLUMN_PK_ID, TABLENAME_SCORES, TBL_SCORE_COLUMN_FK_UserID, TBL_SCORE_COLUMN_Difficulty, difficulty, TBL_SCORE_COLUMN_Time)
